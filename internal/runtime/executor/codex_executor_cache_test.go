...
func TestCodexExecutorCacheHelper_ClaudeAgentScopeUsesResolvedModelAcrossHTTPAndWebsocket(t *testing.T) {
	...
	websocketBody, _, errWebsocket := applyCodexPromptCacheHeadersWithContext(context.Background(), nil, sdktranslator.FromString("claude"), aliasReq, rawJSON, childHeaders)
	if errWebsocket != nil {
